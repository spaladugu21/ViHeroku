    ...
    foreach ($xmlarray as $key=>$val){
		...
		if(empty($val['notifications']['Notification'][0])){
			...
			foreach($val as $fdata){
                ...
                $readerEvent__c = !empty($fdata['Notification']['sObject']['readerEvent__c']) ? "'str_replace("'","''",$readerEvent__c)'" : "NULL";
                ...
                $Responce_JSON__c = !empty($fdata['Notification']['sObject']['Responce_JSON__c']) ? "'str_replace("'","''",$Responce_JSON__c)'" : "NULL";
                ...
                $Street__c = !empty($fdata['Notification']['sObject']['Street__c']) ? "'str_replace("'","''",$Street__c)'" : "NULL";
		...
		$recordname=$fdata['Notification']['sObject']['RecordType_name__c'];
			...
			$query ="INSERT INTO public.individual_tip_non_tip_events__c (Id, Ammout_Over_Limit_mph__c, Amount_Over_Limit_Km_h__c, Asset_Serial__c, Asset_Type__c, City__c, Collection_Route_Text__c, Collection_Route__c, Container_RFID_tag__c, Country__c, CreatedById, CreatedDate, CreatedDateOnly__c, Customer_Location__c, Distance__c, Duration__c, Error_Code__c, Error_Description__c, Event_End_Date_Time__c, Event_End__Latitude__s, Event_End__Longitude__s, Event_Start_Date_Time__c, Event_Start_Date__c , Event_Start_Tip_Date__c, Event_Start__Latitude__s, Event_Start__Longitude__s, HarshAcceleration__c, HarshBrakingInsyncId__c, Heading__c, HeartbeatInsyncId__c, House_Number__c, IdleTimeInsyncId__c, InsyncId__c, Is_GPS_Location__c, Is_Tipped_Twice__c, Is_Viewable_On_Map__c, LastModifiedById, LastModifiedDate, Location_Address__c, Max_Speed_Miles_per_hour__c, Name, ObservationInsyncId__c, Observation_Description__c, Observation_Number__c, Obs_Num_Desc__c, OwnerId, readerEvent__c, RecordTypeId, Responce_JSON__c, Service_Provider__c, SFDC_Container_Asset__c, SFDC_Device_Id__c, SFDC_Truck_ID__c, SpeedingInsyncId__c, State__c, Street__c, Telematic_Event_Date__c, Timezone__c, TipNoTipNumber__c, Tip_DateTime__c, Tip_Date_Time__c, Tip_Date__c, Tip_hour_range__c, Tip_Hour__c, Tip_Lat_Lon__Latitude__s, Tip_Lat_Lon__Longitude__s, Tip_or_Non_Tip__c, Truck_No__c, Truck_Type__c, Zip_Code__c,Tiporigin_heroku__c,heroku_created_date,recordname) VALUES ($Id,$Ammout_Over_Limit_mph__c,$Amount_Over_Limit_Km_h__c,$Asset_Serial__c,$Asset_Type__c,$City__c,$Collection_Route_Text__c,$Collection_Route__c,$Container_RFID_tag__c,$Country__c,$CreatedById,$CreatedDate,$CreatedDateOnly__c,$Customer_Location__c,$Distance__c,$Duration__c,$Error_Code__c,$Error_Description__c,$Event_End_Date_Time__c,$Event_End__Latitude__s,$Event_End__Longitude__s,$Event_Start_Date_Time__c,$Event_Start_Date__c ,$Event_Start_Tip_Date__c,$Event_Start__Latitude__s,$Event_Start__Longitude__s,$HarshAcceleration__c,$HarshBrakingInsyncId__c,$Heading__c,$HeartbeatInsyncId__c,$House_Number__c,$IdleTimeInsyncId__c,$InsyncId__c,$Is_GPS_Location__c,$Is_Tipped_Twice__c,$Is_Viewable_On_Map__c,$LastModifiedById,$LastModifiedDate,$Location_Address__c,$Max_Speed_Miles_per_hour__c,$Name,$ObservationInsyncId__c,$Observation_Description__c,$Observation_Number__c,$Obs_Num_Desc__c,$OwnerId,$readerEvent__c,$RecordTypeId,$Responce_JSON__c,$Service_Provider__c,$SFDC_Container_Asset__c,$SFDC_Device_Id__c,$SFDC_Truck_ID__c,$SpeedingInsyncId__c,$State__c,$Street__c,$Telematic_Event_Date__c,$Timezone__c,$TipNoTipNumber__c,$Tip_DateTime__c,$Tip_Date_Time__c,$Tip_Date__c,$Tip_hour_range__c,$Tip_Hour__c,$Tip_Lat_Lon__Latitude__s,$Tip_Lat_Lon__Longitude__s,$Tip_or_Non_Tip__c,$Truck_No__c,$Truck_Type__c,$Zip_Code__c,$TipOrigin_heroku__c,$hdate,$recordname)";
			...
			foreach($val['notifications']['Notification'] as $fdata){
                ...
                $Location_Address__c = !empty($fdata['sObject']['Location_Address__c']) ? "'str_replace("'","''",$Location_Address__c)'" : "NULL";
                ...
                $readerEvent__c = !empty($fdata['sObject']['readerEvent__c']) ? "'str_replace("'","''",$readerEvent__c)'" : "NULL";
                ...
                $Responce_JSON__c = !empty($fdata['sObject']['Responce_JSON__c']) ? "'str_replace("'","''",$Responce_JSON__c)'" : "NULL";
