...
if(!empty($xmlarray) && $xmlarray!=='no_record'){
    foreach ($xmlarray as $key=>$val){
		...
		$hdate = "'" .date("Y-m-d h:i:sa T"). "'";
			...
			$truck_segment__c =$fdata['Notification']['sObject']['truck_segment__c'];
                $truck_segment__c = !empty($fdata['Notification']['sObject']['truck_segment__c']) ? "'$truck_segment__c'" : "NULL";
